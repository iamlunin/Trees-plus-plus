	...
	CellularAutomation(int w, int h) {
		...
		for (int ind = 0; ind < world_map.size(); ind++) {
			...
			if (y < 1 || float(y)/height < 
				...
				(sin(x * 0.1747567935 + 346) / 2 + 0.5) * 0.007 +
				(sin(x * 0.3436463457 + 2123534) / 2 + 0.5) * 0.005 +
				(sin(x * 0.745365236 + 1643) / 2 + 0.5) * 0.003 
	...
	void spawn_starting_seeds_if_needed() {
		...
		if (live_cell_arr.size() == 0) {
			...
			for (int i = 0; i < width / 4; i++)
				spawn(rand() % width,  rand() % height / 5 );
		}
	}
