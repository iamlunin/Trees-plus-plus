...
#undef OP_TYPE
	...
	constexpr inline Vector<T1, size> clamp(const Vector<T1, size> &v, const Vector<T1, size> &lo, const Vector<T1, size> &hi) {
		Vector<T1, size> out((p_v_unused()));
		for (size_t i = 0; i < size; i++) {
			const auto &value = v[i];
			const auto &l2 = lo[i];
			const auto &h2 = hi[i];
			out[i] = value < l2 ? l2 : (value > h2 ? h2 : value);
		}
		return out;
	}

	template<typename T1, size_t size>
	constexpr inline Vector<T1, size> mix(const Vector<T1, size> &a, const Vector<T1, size> &b, const Vector<T1, size> &k) {
		Vector<T1, size> out((p_v_unused()));
		for (size_t i = 0; i < size; i++) {
			const auto &_a = a[i];
			const auto &_b = b[i];
			const auto &_k = k[i];
			out[i] = _a + (_b - _a) * _k;//value < l2 ? l2 : (value > h2 ? h2 : value);
		}
		return out;
	}
